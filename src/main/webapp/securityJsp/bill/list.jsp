	...
	<div class="main">
		<div class="optitle clearfix">
			<c:if test="${sessionScope.user.role=='300'}">
				<em> <input type="button" name="button" value="添加数据" class="input-button" onclick="window.location.href='bill_addParam.action'" />
				</em>
			</c:if>
			<div class="title">
				账单管理&gt;&gt;
		...
		<div class="content">
			<table class="list">
				...
				<c:forEach var="bill" items="${billList.datas}" varStatus="vars">
					<tr>
						...
						<c:if test="${sessionScope.user.role=='300'}">
							<td align="center">
								<button
									onclick="window.location.href='bill_updateParam.action?id=${bill.bid}'">修改</button>
								<button
									onclick="if(window.confirm( '确定要删除[${bill.productName}]吗?')==true){
												window.location.href='bill_delete.action?id=${bill.bid}'}">
									删除</button>
							</td>
						</c:if>
					</tr>
				...
				<tr>
					<td colspan="9">
						<jsp:include page="../inc/pager.jsp">
							...
							<jsp:param value="${billList.totalRecord }" name="items" />
